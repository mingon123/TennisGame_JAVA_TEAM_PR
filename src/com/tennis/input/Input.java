...
public class Input {
	...
	public String gameTypeSelector() { // 게임타입 입력받는라인
		...
		while (true) {
			...
			System.out.println("단식 / 복식을 선택해 주십시오 ");
	...
	public int gameCountSelector(String gameType) { // 경기수 선택받는 라인
		if (gameType.equals("단식")) {
			...
			if(gender.equals("남자")) { // 이부분 최적화 가능
				System.out.println("3경기 / 5경기 중 하나를 선택해 주십시오 ex) 3경기 -> 3");
				int count;
		...
		System.out.println("aaagame_type" );
		return -1;
	}
	...
	public String genderSelector() { // 성별타입 입력받는라인
		...
		while (true) {
			Print.printRuleSelect();
			System.out.println("참가하는 선수의 성별을 선택해 주십시오 (남자/여자)");
	...
	public static final List<Player> malePlayers = new ArrayList<>();
	public static final List<Player> femalePlayers = new ArrayList<>();
	...
	public List<Player> playerSetting(String gameType) {
		...
		System.out.println("aaaaaaa playersetting");
	...
	public Team[] teamSetting(List<Player> players) { // players List를 Team 배열로 반환
